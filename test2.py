...
def open_medication_history():
    ...
    columns = ("Disease", "Blood Group", "OH Level", "Diabetes", "BMI", "Age", "X-Ray Link", "DateTime")
    ...
    def refresh_history():
        ...
        conn = sqlite3.connect(DB_NAME)
        cur = conn.cursor()
        cur.execute("SELECT disease, blood_group, oh_level, diabetes, bmi, age, xray_link, timestamp FROM medication_history WHERE uid=? ORDER BY timestamp DESC", (uid,))
        for row in cur.fetchall():
            tree.insert('', tk.END, values=row)
        conn.close()
    ...
    def add_history():
        values = (
            disease_entry.get().strip(),
            bg_entry.get().strip(),
            oh_entry.get().strip(),
            diabetes_entry.get().strip(),
            bmi_entry.get().strip(),
            age_entry.get().strip(),
            xray_entry.get().strip(),
            datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        )
        conn = sqlite3.connect(DB_NAME)
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO medication_history (uid, disease, blood_group, oh_level, diabetes, bmi, age, xray_link, timestamp)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (uid,) + values)
        conn.commit()
        conn.close()
    ...
    disease_entry = ttk.Entry(frm, width=15)
    bg_entry = ttk.Entry(frm, width=10)
    oh_entry = ttk.Entry(frm, width=10)
    diabetes_entry = ttk.Entry(frm, width=10)
    bmi_entry = ttk.Entry(frm, width=10)
    age_entry = ttk.Entry(frm, width=10)
    xray_entry = ttk.Entry(frm, width=30)

    entries = [
        ("Disease", disease_entry),
        ("Blood Group", bg_entry),
        ("OH", oh_entry),
        ("Diabetes", diabetes_entry),
        ("BMI", bmi_entry),
        ("Age", age_entry),
        ("X-Ray", xray_entry)
    ]

    for i, (lbl, entry) in enumerate(entries):
        ttk.Label(frm, text=lbl + ":").grid(row=0, column=i*2)
        entry.grid(row=0, column=i*2+1, padx=2)

    ttk.Button(win, text="Add", command=add_history).pack(pady=5)
